...
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply('**Aleyküm Selam Hoşgeldin** <a:altin2:754824157539074108>');    
      }
      }
    });
...
client.on('message', async (msg, member, guild) => {
  let i = await  db.fetch(`saas_${msg.guild.id}`)
      if(i === 'açık') {
        ...
        msg.reply('**Hi Welcome!** <a:altin2:754824157539074108> ');    
      }
      }
    });
...
client.on(`guildMemberAdd`, async member => {
  if(member.guild.id !== '731977708489146458') return;
  const e = new Discord.RichEmbed()
    .setColor(`RED`)
    .setImage(`https://i.gifer.com/UHeN.gif`)
