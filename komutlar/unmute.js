...
const db = require('quick.db')
  ...
  if (!modlog) return message.reply('Mod log kanalı ayarlamamışsınız.');
    ...
    if (!user.roles.find(`name`, "Leasess | Susturma")) return message.channel.send('Kişi Mutelenmemiş')
    let muterole = message.guild.roles.find(`name`, "Leasess | Susturma");

    if (!muterole) {
        try {
           muterole = await message.guild.createRole({
                name: "Leasess | Susturma",
                color: "#000000",
                permissions: []
            })
